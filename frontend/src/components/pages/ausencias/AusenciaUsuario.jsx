...
export function AusenciaUsuario() {
    ...
    const handleCrearAusencia = () => {
        ...
        console.log("(dayjs(fechaInicio).isSame(dayjs(fechaFin)) && dayjs(horaInicio).isAfter(dayjs(horaFin)))")
        ...
        fetch(`http://localhost:8000/guardias?${queryParams}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: "Bearer " + token,
                ...
                <div className="flex flex-col flex-wrap gap-5">
                    <div className="flex flex-wrap flex-row gap-2">
                        ...
                        <Typography variant="lead" color="black" size="xl" className="mr-auto">
                            {usuario?.nombre ? usuario.nombre : "No se ha podido obtener el usuario"}
                        </Typography>
                    </div>
                    <div className="flex flex-wrap flex-row gap-2">
                        <Typography variant="h4" color="black" size="xl">
                        ...
                        <LocalizationProvider dateAdapter={AdapterDayjs}>
                            <DatePicker
                                ...
                                onChange={(date) => handleCabioFecha(date, setFechaInicio)}
                                minDate={minDate}
                                maxDate={maxDate}
                            ...
                            <TimePicker
                                views={["hours", "minutes"]}
                                onChange={(hour) => handleCabioHora(hour, setHoraInicio)}
                                className="sm:w-full md:w-1/3 lg:w-1/4"
                                minTime={minHour}
                                maxTime={maxHour}
                            />
                    ...
                    <div className="flex flex-wrap flex-row gap-2">
                        ...
                        <LocalizationProvider dateAdapter={AdapterDayjs}>
                            <DatePicker
                                disablePast
                                views={["year", "month", "day"]}
                                onChange={(date) => handleCabioFecha(date, setFechaFin)}
                                minDate={minDate}
                                maxDate={maxDate}
                                className="sm:w-full md:w-1/3 lg:w-1/4"
                            />
                            <TimePicker
                                views={["hours", "minutes"]}
                                onChange={(hour) => handleCabioHora(hour, setHoraFin)}
                                className="sm:w-full md:w-1/3 lg:w-1/4"
                                minTime={minHour}
                                maxTime={maxHour}
                            />
